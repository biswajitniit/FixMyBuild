...
class TradepersionDashboardController extends Controller
{
    ...
    public function save_project_estimate(Request $request, $id)
    {
        ...
        try{
            $old_estimate = Estimate::where([
                                'project_id'      => Hashids_decode($request->id),
                                'tradesperson_id' => Auth::user()->id,
                            ])->first();
            ...
            if (Str::lower($request->describe_mode) == 'unable_to_describe') {
                ...
                Estimate::create([
                    ...
                    'project_id'              => Hashids_decode($request->id)[0],
                    'tradesperson_id'         => Auth::user()->id,
    ...
    public function details($id)
    {
        ...
        if($status_proj == 'estimate_submitted' || $status_proj == 'project_started' || $status_proj == 'estimate_accepted' || $status_proj == 'estimate_rejected' || $status_proj == 'estimate_recalled' || $status_proj == 'project_paused' || $status_proj == 'project_cancelled' || $status_proj == 'estimate_not_accepted') {
